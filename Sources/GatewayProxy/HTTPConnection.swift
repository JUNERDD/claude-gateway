...
final class HTTPConnection {
    ...
    init(fd: Int32, localMasterKey: String) {
        self.fd = fd
    ...
    private func forwardMessages(_ request: HTTPRequest) {
        ...
        let route = settings.route(for: originalModel)
        guard let provider = settings.provider(id: route.providerID) else {
            writeLoggedJSON(status: 502, payload: [
                "error": [
                    "type": "upstream_error",
                    "message": "Provider \(route.providerID) is not configured.",
                ],
            ], request: request)
            return
        }

        let targetModel = route.upstreamModel
        if !targetModel.isEmpty {
            payload["model"] = targetModel
        }
        if let originalModel, originalModel != targetModel {
        ...
        let providerBaseURL = provider.baseURL.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        guard !providerBaseURL.isEmpty else {
            writeLoggedJSON(status: 502, payload: [
                "error": [
                    "type": "upstream_error",
                    "message": "Provider \(provider.nameForDisplay) base URL is not configured.",
                ],
            ], request: request)
            return
        }

        let targetURLString = providerBaseURL + request.path
        ...
        let providerSecret = secrets.providerSecrets[provider.id]?.apiKey.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if provider.auth.requiresAPIKey, providerSecret.isEmpty {
            writeLoggedJSON(status: 502, payload: [
                "error": [
                    "type": "upstream_error",
                    "message": "Provider \(provider.nameForDisplay) API key is not configured.",
                ],
            ], request: request)
            return
        }
        ...
        applyProviderAuth(provider.auth, apiKey: providerSecret, to: &upstream)
    ...
    private func applyProviderAuth(_ auth: GatewayProviderAuth, apiKey: String, to request: inout URLRequest) {
        switch auth.type {
        case GatewayProviderAuth.bearer:
            request.setValue("Bearer \(apiKey)", forHTTPHeaderField: "authorization")
        case GatewayProviderAuth.none:
            return
        case GatewayProviderAuth.customHeader:
            let header = auth.customHeaderName.trimmingCharacters(in: .whitespacesAndNewlines)
            if !header.isEmpty {
    ...
    private func readRequest() throws -> HTTPRequest {
        ...
        while buffer.range(of: delimiter) == nil {
            ...
            if count <= 0 {
                throw NSError(domain: "GatewayProxy", code: 1, userInfo: [NSLocalizedDescriptionKey: "Empty HTTP request"])
            }
            buffer.append(temp, count: count)
            if buffer.count > 2 * 1024 * 1024 {
                throw NSError(domain: "GatewayProxy", code: 2, userInfo: [NSLocalizedDescriptionKey: "HTTP headers are too large"])
            }
        }
        ...
        guard let headerRange = buffer.range(of: delimiter),
            ...
            throw NSError(domain: "GatewayProxy", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid HTTP headers"])
        }

        let lines = headerText.split(separator: "\r\n", omittingEmptySubsequences: false).map(String.init)
        guard let requestLine = lines.first else {
            throw NSError(domain: "GatewayProxy", code: 4, userInfo: [NSLocalizedDescriptionKey: "Missing request line"])
        }
        let requestParts = requestLine.split(separator: " ", maxSplits: 2).map(String.init)
        guard requestParts.count >= 2 else {
            throw NSError(domain: "GatewayProxy", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid request line"])
        }
