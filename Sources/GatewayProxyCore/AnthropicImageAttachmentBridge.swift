...
public struct AnthropicImageAttachmentBridgeConfiguration {
    ...
    public static func defaultCacheDirectory() -> URL {
        ...
        return base.appendingPathComponent("ClaudeGateway", isDirectory: true)
    }
}
...
public final class AnthropicImageAttachmentBridge {
    ...
    private func savedTextBlock(imageIndex: Int, mimeType: String, byteCount: Int, path: String) -> [String: Any] {
        ...
        The original Anthropic image block was saved as a local file because the configured downstream model may be text-only.
        Path: \(path)
        MIME type: \(mimeType)
    ...
    private func fallbackBlock(
        ...
        The original image was not sent upstream because the configured text provider may not support multimodal input.
        Raw error: \(reason)
        """)
    }
