...
enum ClaudeDesktopConfigSync {
    ...
    private static func bundledClaudeMCPServersSourceURL() -> URL? {
        let fm = FileManager.default
        var candidates: [URL] = []
        if let override = ProcessInfo.processInfo.environment["CLAUDE_GATEWAY_CLAUDE_MCP_SOURCE"],
    ...
    private static func claudeMCPServersDestinationRoot() -> URL {
        ...
        if let override = environment["CLAUDE_GATEWAY_CLAUDE_MCP_DESTINATION"],
            !override.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        {
            return URL(fileURLWithPath: expandTilde(override), isDirectory: true)
        }
        if let claudeHome = environment["CLAUDE_GATEWAY_CLAUDE_HOME"],
    ...
    private static func visionMCPServerConfig(settings: ProxyDiskSettings, localGatewayKey: String) -> [String: Any]? {
        ...
        return [
            ...
            "args": [serverURL.path],
            "env": [
                "CLAUDE_GATEWAY_URL": "http://\(settings.host):\(settings.port)",
    ...
    static func targetClaudeDesktopMCPConfigURLs(
        ...
        if let override = environment["CLAUDE_GATEWAY_CLAUDE_DESKTOP_CONFIG"],
            !override.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        {
