...
enum BundledRuntimeInstaller {
    static var configDirURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/claude-gateway", isDirectory: true)
    }
    ...
    static var startScriptURL: URL {
        binDirURL.appendingPathComponent("claude-gateway-start.sh")
    }
    ...
    static func runtimeLooksInstalled() -> Bool {
        ...
        return requiredFiles.allSatisfy { fm.fileExists(atPath: $0) }
            ...
            && fm.isExecutableFile(atPath: binDirURL.appendingPathComponent("claude-gateway-proxy.sh").path)
            && fm.isExecutableFile(atPath: binDirURL.appendingPathComponent("claude-gateway-doctor.sh").path)
            && fm.isExecutableFile(atPath: configDirURL.appendingPathComponent("gateway_proxy").path)
    }
    ...
    static func installOrRepair() throws -> RuntimeInstallReport {
        ...
        let files: [(String, URL, UInt16)] = [
            ("claude-gateway-start.sh", binDirURL.appendingPathComponent("claude-gateway-start.sh"), 0o755),
            ("claude-gateway-proxy.sh", binDirURL.appendingPathComponent("claude-gateway-proxy.sh"), 0o755),
            ("claude-gateway-doctor.sh", binDirURL.appendingPathComponent("claude-gateway-doctor.sh"), 0o755),
            ("gateway_proxy", configDirURL.appendingPathComponent("gateway_proxy"), 0o755),
        ]
