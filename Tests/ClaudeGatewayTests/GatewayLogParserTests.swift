...
final class GatewayLogParserTests: XCTestCase {
    func testStructuredErrorResponseIsPromotedToWarningEvent() throws {
        let logText = """
        CG_EVENT {"durationMs":1200,"outputTokensEstimate":88,"providerID":"custom","requestID":"req-429","responseBodyBytes":512,"status":429,"timestamp":"2026-05-07T10:00:00Z","type":"gateway_response"}
        """

        let event = try XCTUnwrap(GatewayLogParser.parse(logText).first)

        XCTAssertEqual(event.title, "Gateway 响应")
    ...
    func testPlainFailureLineUsesErrorTone() throws {
        let event = try XCTUnwrap(GatewayLogParser.parse("操作失败：Provider API Key 不能为空。").first)

        XCTAssertEqual(event.title, "操作失败：Provider API Key 不能为空。")
    ...
    func testGeminiVisionPreprocessEventShowsFallbackWarning() throws {
        let logText = """
        CG_EVENT {"durationMs":99,"fallbackCount":1,"imageCount":2,"model":"gemini-2.5-flash","requestID":"req-vision","successCount":1,"timestamp":"2026-05-07T10:00:00Z","type":"gemini_vision_preprocess"}
        """
    ...
    func testImageAttachmentBridgeEventShowsSavedCount() throws {
        let logText = """
        CG_EVENT {"attachments":[{"byteCount":12,"imageIndex":1,"mimeType":"image/png","path":"/tmp/image-1.png","status":"saved"}],"fallbackCount":0,"imageCount":1,"requestID":"req-bridge","savedCount":1,"timestamp":"2026-05-07T10:00:00Z","totalBytes":12,"type":"image_attachment_bridge"}
        """
    ...
    func testVisionGatewayResponseEventShowsProviderAndModel() throws {
        let logText = """
        CG_EVENT {"durationMs":321,"imageBytes":80288,"model":"qwen3-vl-flash","provider":"dashscope","requestID":"req-vision-mcp","responseBodyBytes":512,"status":200,"timestamp":"2026-05-07T10:00:00Z","type":"vision_gateway_response"}
        """
