...
4. Save.
5. Copy the Claude Desktop config snippet from Settings into Claude Desktop's gateway configuration.
6. Start the gateway from the main window.

Claude DeepSeek Gateway can install and run the local gateway, but it cannot automatically register that gateway inside Claude Desktop. Claude Desktop must be told which local URL, bearer key, and model names to use; that is why the app generates a config snippet for you to copy.
...
This manual Claude Desktop configuration step is required because the gateway runs as a local service outside Claude Desktop. Claude Desktop does not discover arbitrary local gateways on its own; it needs an explicit `inferenceGatewayBaseUrl`, auth scheme, key, and model list.
