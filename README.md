...
Saving settings installs/updates the local LaunchAgent, starts the gateway, refreshes Claude's gateway model cache, and syncs Claude-3p's config library with the correct URL, bearer key, and visible model list. The main window also auto-starts the gateway on launch when a DeepSeek API key is already configured.
